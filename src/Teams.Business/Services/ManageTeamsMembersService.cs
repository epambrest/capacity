...
namespace Teams.Business.Services
{
    public class ManageTeamsMembersService : IManageTeamsMembersService
    {
        ...
        private readonly IRepository<TeamMemberBusiness, int> _teamMemberRepository;

        public ManageTeamsMembersService(IRepository<TeamBusiness, int> teamRepository, 
            IRepository<TeamMemberBusiness, int> memberRepository, ICurrentUser currentUser)
        {
        ...
        public async Task<bool> RemoveAsync(int teamId, string memberId)
        {
            ...
            if (member != null) return await _teamMemberRepository.DeleteAsync(member);
            return false;
        }
        ...
        public async Task<bool> AddAsync(int teamId, string memberId)
        {
            ...
            if (!alreadyInTeam && memberId != _currentUser.Current.Id())
                return await _teamMemberRepository.InsertAsync(new TeamMemberBusiness { TeamId = teamId, MemberId = memberId });
            return false;
        }
        ...
        public async Task<List<TeamMemberBusiness>> GetAllTeamMembersAsync(int teamId, DisplayOptions options)
        {
            var allMembers = await _teamMemberRepository.GetAllAsync();
